...
<head>
    <meta charset="utf-8">
    <title>Kartu Kendali Fire Alarm - {{ $fireAlarm->barcode }}</title>
...
<div class="no-print bg-white border-b shadow-sm sticky top-0 z-10">
    <div class="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
        <div>
            <h1 class="text-lg font-bold text-gray-900">Kartu Kendali Fire Alarm</h1>
            <p class="text-sm text-gray-600">{{ $fireAlarm->barcode ?? $fireAlarm->serial_no }}</p>
        </div>
        <div class="flex gap-2">
            <a href="{{ route('fire-alarm.index') }}" class="px-4 py-2 border rounded-lg hover:bg-gray-50 text-sm font-medium">
                ← Kembali
            </a>
...
<div class="max-w-5xl mx-auto px-4 py-6">
    <div class="sheet-a4 bg-white rounded-xl shadow-lg border p-8">
        ...
        <div class="flex items-start justify-between mb-6 pb-4 border-b-2 border-gray-200">
            <div class="flex-1">
                <h2 class="text-2xl font-bold text-gray-900 mb-2">KARTU KENDALI FIRE ALARM</h2>
                <div class="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
                    <div>
                        <span class="text-gray-600">Kode/Barcode:</span>
                        <span class="font-semibold ml-2">{{ $fireAlarm->barcode ?? $fireAlarm->serial_no }}</span>
                    </div>
                    <div>
        ...
        <div class="mb-6 p-4 bg-slate-50 rounded-lg border border-slate-200">
            ...
            <div class="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
                <div>
                    ...
                    <span class="font-semibold ml-2">{{ $fireAlarm->name ?? '-' }}</span>
                </div>
                <div>
                    <span class="text-gray-600">No. Seri:</span>
                    <span class="font-semibold ml-2">{{ $fireAlarm->serial_no ?? '-' }}</span>
                </div>
                <div>
                    <span class="text-gray-600">Lokasi:</span>
                    <span class="font-semibold ml-2">{{ $fireAlarm->location_code ?? '-' }}</span>
                ...
                <div>
                    ...
                    <span class="font-semibold ml-2">{{ $fireAlarm->zone ?? '-' }}</span>
                </div>
            </div>
        </div>
