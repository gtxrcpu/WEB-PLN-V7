...
<x-layouts.app :title="'Riwayat Inspeksi Box Hydrant ' . ($boxHydrant->serial_no ?? '')">
    ...
    <div class="mb-4">
        <a href="{{ route('box-hydrant.index') }}"
           class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white border-2 border-slate-200 text-slate-700 text-sm font-medium hover:bg-slate-50 hover:border-slate-300 transition-all duration-200 shadow-sm">
            ...
            <span>Kembali ke Daftar Box Hydrant</span>
        </a>
    </div>

    {{-- Header --}}
    <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-600 via-cyan-600 to-blue-700 p-8 shadow-xl">
        ...
        <div class="relative">
            <div class="flex items-center gap-4 mb-4">
                ...
                <div>
                    ...
                    <p class="text-blue-100 text-sm">Box Hydrant {{ $boxHydrant->barcode ?? $boxHydrant->serial_no }}</p>
                </div>
            </div>
            <div class="grid grid-cols-3 gap-4 mt-6">
                <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4">
                    <p class="text-blue-100 text-xs mb-1">Lokasi</p>
                    <p class="text-white font-semibold">{{ $boxHydrant->location_code ?? '-' }}</p>
                </div>
                <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4">
                    <p class="text-blue-100 text-xs mb-1">Tipe</p>
                    <p class="text-white font-semibold">{{ $boxHydrant->type ?? '-' }}</p>
                </div>
                <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4">
                    <p class="text-blue-100 text-xs mb-1">Total Inspeksi</p>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
        ...
        <form method="GET" action="{{ route('box-hydrant.riwayat', $boxHydrant->id) }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div class="flex items-end">
                <button type="submit" 
                        class="w-full px-4 py-2 bg-gradient-to-r from-blue-600 to-cyan-600 text-white rounded-lg hover:from-blue-700 hover:to-cyan-700 transition-colors text-sm font-semibold flex items-center justify-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="p-6">
                        <div class="flex items-start justify-between gap-4 mb-4">
                            <div class="flex items-center gap-3">
                                <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-blue-500 to-cyan-500 flex items-center justify-center text-white font-bold shadow-lg">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4">
                            <div class="rounded-xl bg-slate-50 p-3 border border-slate-100">
                                ...
                                <p class="text-sm font-semibold {{ $kartu->pilar_hydrant === 'baik' ? 'text-emerald-600' : 'text-rose-600' }}">
                                    {{ ucfirst(str_replace('_', ' ', $kartu->pilar_hydrant)) }}
                                </p>
                            </div>
                            <div class="rounded-xl bg-slate-50 p-3 border border-slate-100">
                                <p class="text-xs text-slate-500 mb-1">Pin/Segel</p>
                                <p class="text-sm font-semibold {{ $kartu->box_hydrant === 'baik' ? 'text-emerald-600' : 'text-rose-600' }}">
                                    {{ ucfirst(str_replace('_', ' ', $kartu->box_hydrant)) }}
                                </p>
                            </div>
                            <div class="rounded-xl bg-slate-50 p-3 border border-slate-100">
                            ...
                            <div class="rounded-xl bg-slate-50 p-3 border border-slate-100">
                                ...
                                <p class="text-sm font-semibold {{ $kartu->nozzle === 'baik' ? 'text-emerald-600' : 'text-rose-600' }}">
                                    {{ ucfirst(str_replace('_', ' ', $kartu->nozzle)) }}
                                </p>
                            </div>
                            <div class="rounded-xl bg-slate-50 p-3 border border-slate-100">
                                <p class="text-xs text-slate-500 mb-1">selang</p>
    ...
    @else
        <div class="relative rounded-2xl border-2 border-dashed border-slate-300 p-12 text-center bg-gradient-to-br from-slate-50 to-white">
            ...
            <p class="text-slate-600 text-sm mb-6">
                Box Hydrant ini belum pernah diinspeksi. Buat kartu kendali untuk memulai inspeksi.
            </p>
