{{-- resources/views/box-hydrant/edit.blade.php --}}
@extends('layouts.app')

@section('title', 'Edit Box Hydrant ' . ($boxHydrant->serial_no ?? ''))

@section('content')
<div class="max-w-4xl mx-auto px-4 py-6 space-y-6">
    {{-- Header Section --}}
    <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-600 via-cyan-600 to-rose-600 p-8 shadow-xl">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-xl overflow-hidden">
        <div class="bg-gradient-to-r from-slate-50 to-slate-100 px-8 py-5 border-b border-slate-200">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-lg font-bold text-slate-900">Informasi Box Hydrant</h2>
                    <p class="text-xs text-slate-600">Update data Box Hydrant yang sudah ada</p>
                </div>
            </div>
        </div>

        <form method="POST" action="{{ route('box-hydrant.update', $boxHydrant) }}" class="p-8">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div class="grid md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" value="{{ $boxHydrant->barcode }}" disabled
                               class="block w-full rounded-xl border-2 border-slate-200 bg-gradient-to-br from-slate-50 to-slate-100 text-slate-900 text-base font-mono font-bold px-4 py-3 shadow-sm">
                    </div>
                </div>
                ...
                <p class="text-xs text-slate-500 flex items-center gap-1.5 -mt-3">
                    ...
                    Serial dan Barcode tidak dapat diubah setelah Box Hydrant dibuat
                </p>
                ...
                <div>
                    ...
                    <input type="text" name="name"
                           value="{{ old('name', $boxHydrant->name) }}"
                           placeholder="Contoh: Box Hydrant F1.001"
                ...
                <div class="grid md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="location_code"
                               ...
                               class="block w-full rounded-xl border-2 border-slate-200 text-slate-900 px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all @error('location_code') border-rose-500 @enderror">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="type"
                               ...
                               class="block w-full rounded-xl border-2 border-slate-200 text-slate-900 px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all @error('type') border-rose-500 @enderror">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="zone"
                               value="{{ old('zone', $boxHydrant->zone) }}"
                               placeholder="Misal: Zone 1 / Zone A"
                               class="block w-full rounded-xl border-2 border-slate-200 text-slate-900 px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all @error('zone') border-rose-500 @enderror">
                    </div>
                            ...
                            @foreach([
                                ...
                                <label class="relative cursor-pointer group">
                                    <input type="radio" name="status" value="{{ $status }}"
                                           {{ old('status', $boxHydrant->status) === $status ? 'checked' : '' }}
                                           class="peer sr-only">
                ...
                <div>
                    ...
                    <textarea name="notes" rows="4"
                              ...
                              class="block w-full rounded-xl border-2 border-slate-200 text-slate-900 px-4 py-3 text-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all resize-none @error('notes') border-rose-500 @enderror">{{ old('notes', $boxHydrant->notes) }}</textarea>
                </div>

                {{-- Floor Plan Coordinate Picker --}}
                <x-floor-plan-picker 
                    :floor-plan-id="old('floor_plan_id', $boxHydrant->floor_plan_id)"
                    :floor-plan-x="old('floor_plan_x', $boxHydrant->floor_plan_x)"
                    :floor-plan-y="old('floor_plan_y', $boxHydrant->floor_plan_y)"
                    equipment-type="box_hydrant"
                />
            </div>
