<x-kartu-layout 
    title="Kartu Kendali Box Hydrant" 
    ...
    <div class="mb-6 p-4 bg-slate-50 rounded-lg border border-slate-200">
        <h3 class="font-bold text-gray-900 mb-3">Informasi Box Hydrant</h3>
        <div class="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
            <div>
                <span class="text-gray-600">Nama Barang:</span>
                <span class="font-semibold ml-2">{{ $boxHydrant->name ?? 'PILAR HYDRANT' }}</span>
            </div>
            <div>
                <span class="text-gray-600">No. Seri:</span>
                <span class="font-semibold ml-2">{{ $boxHydrant->serial_no ?? 'BH.001' }}</span>
            </div>
            <div>
    ...
    <form method="POST" action="{{ route('box-hydrant.kartu.store') }}" class="mt-4 space-y-4">
        @csrf
        <input type="hidden" name="fire_alarm_id" value="{{ $boxHydrant->id }}">
        ...
        <div class="no-print pt-4 mt-2 border-t border-dashed border-slate-200 flex items-center justify-between gap-3 text-xs">
            <p class="text-slate-500">
                Data Kartu Kendali akan disimpan dan bisa dicetak ulang dari modul Box Hydrant.
            </p>
            <div class="flex gap-2">
                ...
                <button type="submit"
                        class="px-4 py-2 rounded-lg bg-gradient-to-r from-blue-600 to-cyan-600 text-white text-sm font-medium hover:from-blue-700 hover:to-cyan-700">
