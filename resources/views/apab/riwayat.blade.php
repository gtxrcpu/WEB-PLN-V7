<x-layouts.app :title="'Riwayat Kartu Kendali — ' . $apab->barcode">
  <div class="mb-6">
    <a href="{{ route('apab.index') }}" class="inline-flex items-center gap-2 text-gray-600 hover:text-gray-900 transition-colors mb-4">
    ...
    <p class="text-sm text-gray-600 mt-1">{{ $apab->barcode ?? $apab->serial_no }} - {{ $apab->lokasi }}</p>
  </div>
          ...
          @forelse($kartuKendali as $kartu)
            <tr class="hover:bg-slate-50 transition-colors">
              ...
              <td class="px-6 py-4 text-right">
                <a href="{{ route('apab.view-kartu', [$apab->id, $kartu->id]) }}" 
