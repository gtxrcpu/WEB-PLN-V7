...
<x-layouts.app :title="'Riwayat Inspeksi APAB ' . ($apab->serial_no ?? '')">
    ...
    <div class="mb-4">
        <a href="{{ route('apab.index') }}"
           class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white border-2 border-slate-200 text-slate-700 text-sm font-medium hover:bg-slate-50 hover:border-slate-300 transition-all duration-200 shadow-sm">
            ...
            <span>Kembali ke Daftar APAB</span>
        </a>
    </div>

    {{-- Header --}}
    <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-red-600 via-orange-600 to-red-700 p-8 shadow-xl">
        ...
        <div class="relative">
            <div class="flex items-center gap-4 mb-4">
                ...
                <div>
                    ...
                    <p class="text-red-100 text-sm">APAB {{ $apab->barcode ?? $apab->serial_no }}</p>
                </div>
            </div>
            <div class="grid grid-cols-3 gap-4 mt-6">
                <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4">
                    <p class="text-red-100 text-xs mb-1">Lokasi</p>
                    <p class="text-white font-semibold">{{ $apab->location_code ?? '-' }}</p>
                </div>
                <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4">
                    <p class="text-red-100 text-xs mb-1">Isi APAB</p>
                    <p class="text-white font-semibold">{{ $apab->isi_apab ?? '-' }}</p>
                </div>
                <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
        <div class="flex items-center justify-between mb-4">
            ...
            @if(request()->hasAny(['creator', 'approver', 'status']))
                <a href="{{ route('apab.riwayat', $apab->id) }}" 
        ...
        <form method="GET" action="{{ route('apab.riwayat', $apab->id) }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
            ...
            <div class="flex items-end">
                <button type="submit" 
                        class="w-full px-4 py-2 bg-gradient-to-r from-red-600 to-orange-600 text-white rounded-lg hover:from-red-700 hover:to-orange-700 transition-colors text-sm font-semibold flex items-center justify-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="p-6">
                        <div class="flex items-start justify-between gap-4 mb-4">
                            <div class="flex items-center gap-3">
                                <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-red-500 to-orange-500 flex items-center justify-center text-white font-bold shadow-lg">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4">
                            <div class="rounded-xl bg-slate-50 p-3 border border-slate-100">
                                ...
                                <p class="text-sm font-semibold {{ $kartu->pressure_gauge === 'baik' ? 'text-emerald-600' : 'text-rose-600' }}">
                                    {{ ucfirst(str_replace('_', ' ', $kartu->pressure_gauge)) }}
                                </p>
                            </div>
                            <div class="rounded-xl bg-slate-50 p-3 border border-slate-100">
                                <p class="text-xs text-slate-500 mb-1">Pin/Segel</p>
                                <p class="text-sm font-semibold {{ $kartu->pin_segel === 'baik' ? 'text-emerald-600' : 'text-rose-600' }}">
                                    {{ ucfirst(str_replace('_', ' ', $kartu->pin_segel)) }}
                                </p>
                            </div>
                            <div class="rounded-xl bg-slate-50 p-3 border border-slate-100">
                                <p class="text-xs text-slate-500 mb-1">Selang</p>
                                <p class="text-sm font-semibold {{ $kartu->selang === 'baik' ? 'text-emerald-600' : 'text-rose-600' }}">
                                    {{ ucfirst(str_replace('_', ' ', $kartu->selang)) }}
                                </p>
                            </div>
                            <div class="rounded-xl bg-slate-50 p-3 border border-slate-100">
                                <p class="text-xs text-slate-500 mb-1">Klem Selang</p>
                                <p class="text-sm font-semibold {{ $kartu->klem_selang === 'baik' ? 'text-emerald-600' : 'text-rose-600' }}">
                                    {{ ucfirst(str_replace('_', ' ', $kartu->klem_selang)) }}
                                </p>
                            </div>
                            <div class="rounded-xl bg-slate-50 p-3 border border-slate-100">
                                <p class="text-xs text-slate-500 mb-1">Handle</p>
                                <p class="text-sm font-semibold {{ $kartu->handle === 'baik' ? 'text-emerald-600' : 'text-rose-600' }}">
                                    {{ ucfirst(str_replace('_', ' ', $kartu->handle)) }}
                                </p>
                            </div>
                            <div class="rounded-xl bg-slate-50 p-3 border border-slate-100">
    ...
    @else
        <div class="relative rounded-2xl border-2 border-dashed border-slate-300 p-12 text-center bg-gradient-to-br from-slate-50 to-white">
            ...
            <p class="text-slate-600 text-sm mb-6">
                APAB ini belum pernah diinspeksi. Buat kartu kendali untuk memulai inspeksi.
            </p>
