...
<x-layouts.app :title="'Riwayat Inspeksi Rumah Pompa ' . ($rumahPompa->serial_no ?? '')">
  <div class="max-w-7xl mx-auto px-4 py-6 space-y-6">
    ...
    <div class="mb-4">
        <a href="{{ route('rumah-pompa.index') }}"
           class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white border-2 border-slate-200 text-slate-700 text-sm font-medium hover:bg-slate-50 hover:border-slate-300 transition-all duration-200 shadow-sm">
            ...
            <span>Kembali ke Daftar Rumah Pompa</span>
        </a>
    </div>
        ...
        <div class="relative">
            <div class="flex items-center gap-4 mb-4">
                ...
                <div>
                    ...
                    <p class="text-purple-100 text-sm">Rumah Pompa {{ $rumahPompa->barcode ?? $rumahPompa->serial_no }}</p>
                </div>
            </div>
            <div class="grid grid-cols-3 gap-4 mt-6">
                <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4">
                    <p class="text-purple-100 text-xs mb-1">Lokasi</p>
                    <p class="text-white font-semibold">{{ $rumahPompa->location_code ?? '-' }}</p>
                </div>
                <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4">
                    <p class="text-purple-100 text-xs mb-1">Tipe</p>
                    <p class="text-white font-semibold">{{ $rumahPompa->type ?? '-' }}</p>
                </div>
                <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4">
                    <p class="text-purple-100 text-xs mb-1">Total Inspeksi</p>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
        ...
        <form method="GET" action="{{ route('rumah-pompa.riwayat', $rumahPompa->id) }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
            ...
            <div class="flex items-end">
                <button type="submit" 
                        class="w-full px-4 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-lg hover:from-purple-700 hover:to-indigo-700 transition-colors text-sm font-semibold flex items-center justify-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="p-6">
                        <div class="flex items-start justify-between gap-4 mb-4">
                            <div class="flex items-center gap-3">
                                <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-purple-500 to-indigo-500 flex items-center justify-center text-white font-bold shadow-lg">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4">
                            <div class="rounded-xl bg-slate-50 p-3 border border-slate-100">
                                <p class="text-xs text-slate-500 mb-1">Pressure Gauge</p>
                                <p class="text-sm font-semibold {{ $kartu->pompa_utama === 'baik' ? 'text-emerald-600' : 'text-rose-600' }}">
                                    {{ ucfirst(str_replace('_', ' ', $kartu->pompa_utama)) }}
                                </p>
                            </div>
                            <div class="rounded-xl bg-slate-50 p-3 border border-slate-100">
                                <p class="text-xs text-slate-500 mb-1">Pin/Segel</p>
                                <p class="text-sm font-semibold {{ $kartu->pompa_cadangan === 'baik' ? 'text-emerald-600' : 'text-rose-600' }}">
                                    {{ ucfirst(str_replace('_', ' ', $kartu->pompa_cadangan)) }}
                                </p>
                            </div>
                            <div class="rounded-xl bg-slate-50 p-3 border border-slate-100">
                                <p class="text-xs text-slate-500 mb-1">jockey_pump</p>
                                <p class="text-sm font-semibold {{ $kartu->jockey_pump === 'baik' ? 'text-emerald-600' : 'text-rose-600' }}">
                                    {{ ucfirst(str_replace('_', ' ', $kartu->jockey_pump)) }}
                                </p>
                            </div>
                            <div class="rounded-xl bg-slate-50 p-3 border border-slate-100">
                                <p class="text-xs text-slate-500 mb-1">Klem jockey_pump</p>
                                <p class="text-sm font-semibold {{ $kartu->klem_jockey_pump === 'baik' ? 'text-emerald-600' : 'text-rose-600' }}">
                                    {{ ucfirst(str_replace('_', ' ', $kartu->klem_jockey_pump)) }}
                                </p>
                            </div>
                            <div class="rounded-xl bg-slate-50 p-3 border border-slate-100">
                                <p class="text-xs text-slate-500 mb-1">uji_fungsi</p>
                                <p class="text-sm font-semibold {{ $kartu->uji_fungsi === 'baik' ? 'text-emerald-600' : 'text-rose-600' }}">
                                    {{ ucfirst(str_replace('_', ' ', $kartu->uji_fungsi)) }}
                                </p>
                            </div>
                            <div class="rounded-xl bg-slate-50 p-3 border border-slate-100">
                                <p class="text-xs text-slate-500 mb-1">Kondisi Fisik</p>
                                <p class="text-sm font-semibold {{ $kartu->kesimpulan === 'baik' ? 'text-emerald-600' : 'text-rose-600' }}">
                                    {{ ucfirst(str_replace('_', ' ', $kartu->kesimpulan)) }}
                                </p>
                            </div>
                        </div>
    ...
    @else
        <div class="relative rounded-2xl border-2 border-dashed border-slate-300 p-12 text-center bg-gradient-to-br from-slate-50 to-white">
            <div class="w-20 h-20 mx-auto mb-6 rounded-2xl bg-gradient-to-br from-purple-500 to-indigo-500 flex items-center justify-center shadow-xl">
            ...
            <p class="text-slate-600 text-sm mb-6">
                Rumah Pompa ini belum pernah diinspeksi. Buat kartu kendali untuk memulai inspeksi.
            </p>
