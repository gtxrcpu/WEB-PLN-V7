N<x-guest.layouts.guest>
    <x-slot name="title">Riwayat Kartu Kendali APAT - Guest Access</x-slot>

    <div class="max-w-7xl mx-auto space-y-6">

        {{-- Back Button --}}
        <x-guest.back-button href="{{ route('guest.apat') }}" />
        ...
        <div class="mb-6">
            ...
            <p class="text-sm text-gray-600">{{ $apat->barcode ?? $apat->serial_no }} - {{ $apat->location_code }}</p>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg ring-1 ring-slate-200 p-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div class="rounded-xl bg-slate-50 p-4 border border-slate-100">
                    ...
                    <p class="text-sm font-semibold text-slate-900">{{ $apat->serial_no ?? '—' }}</p>
                </div>
                <div class="rounded-xl bg-slate-50 p-4 border border-slate-100">
                    <p class="text-xs text-slate-500 mb-1">Tipe</p>
                    <p class="text-sm font-semibold text-slate-900">{{ $apat->type ?? '—' }}</p>
                </div>
                <div class="rounded-xl bg-slate-50 p-4 border border-slate-100">
                    <p class="text-xs text-slate-500 mb-1">Kapasitas</p>
                    <p class="text-sm font-semibold text-slate-900">{{ $apat->capacity ?? '—' }}</p>
                </div>
                <div class="rounded-xl bg-slate-50 p-4 border border-slate-100">
