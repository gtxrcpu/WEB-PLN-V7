<x-guest.layouts.guest>
    <x-slot name="title">Riwayat Kartu Kendali APAR - Guest Access</x-slot>

    <div class="max-w-7xl mx-auto space-y-6">

        {{-- Back Button --}}
        <x-guest.back-button href="{{ route('guest.apar') }}" />
        ...
        <div class="mb-6">
            ...
            <p class="text-sm text-gray-600">{{ $apar->barcode ?? $apar->serial_no }} - {{ $apar->location_code }}</p>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg ring-1 ring-slate-200 p-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div class="rounded-xl bg-slate-50 p-4 border border-slate-100">
                    ...
                    <p class="text-sm font-semibold text-slate-900">{{ $apar->serial_no ?? '—' }}</p>
                </div>
                <div class="rounded-xl bg-slate-50 p-4 border border-slate-100">
