<x-layouts.app :title="'Riwayat Kartu Kendali — ' . $apat->barcode">
  <div class="mb-6">
    <a href="{{ route('apat.index') }}" class="inline-flex items-center gap-2 text-gray-600 hover:text-gray-900 transition-colors mb-4">
    ...
    <p class="text-sm text-gray-600 mt-1">{{ $apat->barcode ?? $apat->serial_no }} - {{ $apat->lokasi }}</p>
  </div>
          ...
          @forelse($riwayatInspeksi as $kartu)
            <tr class="hover:bg-slate-50 transition-colors">
              ...
              <td class="px-6 py-4 text-right">
                <a href="{{ route('apat.view-kartu', [$apat->id, $kartu->id]) }}" 
