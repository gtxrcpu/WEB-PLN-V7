{{-- resources/views/apat/create.blade.php --}}
@extends('layouts.app')

@section('title', 'Tambah APAT')
    ...
    <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-cyan-600 via-sky-600 to-blue-600 p-8 shadow-xl">
        ...
        <div class="relative flex items-start justify-between gap-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-3xl font-bold text-white mb-1">Tambah APAT Baru</h1>
                    <p class="text-cyan-100 text-sm">Input data Alat Pemadam Api Tradisional. Barcode otomatis: <span class="font-semibold">APAT A2.xxx</span></p>
                </div>
            </div>
        ...
        <div class="bg-gradient-to-r from-slate-50 to-slate-100 px-8 py-5 border-b border-slate-200">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-600">Lengkapi form di bawah untuk menambahkan APAT</p>
                </div>
            </div>
        </div>

        <form method="POST" action="{{ route('apat.store') }}" class="p-8">
            @csrf
            ...
            <div class="space-y-6">
                ...
                <p class="text-xs text-slate-500 flex items-center gap-1.5 -mt-3">
                    ...
                    Serial dan Barcode otomatis: A2.xxx (increment). QR Code akan mengikuti barcode ini.
                </p>
                ...
                <div class="grid md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <label class="relative cursor-pointer group">
                                <input type="radio" name="status" value="tidak baik"
                                       {{ old('status') === 'tidak baik' ? 'checked' : '' }}
                                       class="peer sr-only">
                                <div class="flex flex-col items-center gap-2 p-4 rounded-xl border-2 border-slate-200 bg-white transition-all peer-checked:border-rose-500 peer-checked:bg-rose-50 peer-checked:shadow-lg peer-checked:shadow-rose-500/20 hover:border-rose-300">
                                    ...
                                    <span class="text-sm font-semibold text-slate-700 peer-checked:text-rose-700">TIDAK BAIK</span>
                                </div>
                            </label>
                        </div>
                        @error('status')
                ...
                <div>
                    ...
                    <textarea name="notes" rows="4"
                              ...
                              class="block w-full rounded-xl border-2 border-slate-200 text-slate-900 px-4 py-3 text-sm focus:border-cyan-500 focus:ring-4 focus:ring-cyan-500/10 transition-all resize-none @error('notes') border-rose-500 focus:border-rose-500 focus:ring-rose-500/10 @enderror">{{ old('notes') }}</textarea>
                    @error('notes')
                        <p class="mt-2 text-xs text-rose-600 flex items-center gap-1">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
