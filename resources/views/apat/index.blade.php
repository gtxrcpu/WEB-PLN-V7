{{-- resources/views/apat/index.blade.php --}}
<x-layouts.app :title="'APAT — Alat Pemadam Api Tradisional'">
    <div class="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        <div class="space-y-6">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h1
                        class="text-3xl font-bold tracking-tight bg-gradient-to-r from-cyan-600 to-sky-600 bg-clip-text text-transparent">
                        Manajemen APAT
                    </h1>
                ...
                <div class="flex items-center gap-3">
                    <a href="{{ route('apat.create') }}"
                        class="group inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-cyan-600 to-sky-600 text-white text-sm font-semibold hover:from-cyan-700 hover:to-sky-700 shadow-lg shadow-cyan-500/30 hover:shadow-xl hover:shadow-cyan-500/40 transition-all duration-300">
            ...
            {{-- Stats Cards --}}
            @php
                $totalApat = ($apats ?? collect())->count();
                $statusBaik = ($apats ?? collect())->where('status', 'baik')->count();
                $statusTidakBaik = ($apats ?? collect())->where('status', 'tidak baik')->count();
            @endphp

            <div class="grid grid-cols-2 lg:grid-cols-3 gap-4">
                <div
                    class="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-cyan-500 to-cyan-600 p-5 shadow-lg hover:shadow-xl transition-all duration-300">
        ...
        <div class="relative">
            ...
            <input type="text" id="searchInput" placeholder="Cari serial number atau lokasi..."
                class="w-full pl-12 pr-4 py-3 border border-slate-200 rounded-xl bg-white text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition-all"
                onkeyup="filterItems()">
        </div>
        ...
        {{-- Grid APAT --}}
        @if(($apats ?? null) && $apats->count())
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                @foreach($apats as $apat)
                    @php
                        // Raw kode (barcode atau serial_no)
                        $kodePendekRaw = $apat->barcode ?? $apat->serial_no ?? '—';

                        // Bersihkan SEMUA "APAT" di depan, walaupun dobel/triple
                        // "APAT APAT A2.001" -> "A2.001"
                        // "APAT A2.001"      -> "A2.001"
                        // "A2.001"           -> "A2.001"
                        $kodePendek = trim(preg_replace('/^(APAT\s*)+/i', '', $kodePendekRaw));
                    ...
                    <div data-item data-serial="{{ $apat->serial_no }}" data-location="{{ $apat->lokasi }}"
                        ...
                        <div class="p-6">
                            ...
                            <div class="relative mb-5">
                                ...
                                <div
                                    ...
                                    {{-- Download QR Button --}}
                                    <button onclick="downloadQR('{{ $apat->qr_url }}', 'QR_APAT_{{ $kodePendek }}')"
        ...
        @else
            <div
                ...
                <div class="relative">
                    <div
                        class="w-20 h-20 mx-auto mb-6 rounded-2xl bg-gradient-to-br from-cyan-500 to-sky-500 flex items-center justify-center shadow-xl">
                    ...
                    <p class="text-slate-600 text-sm mb-1">
                        Mulai kelola APAT dengan menambahkan unit pertama Anda
                    </p>
                    <p class="text-xs text-slate-500 mb-6">
                        Setelah menambahkan APAT, Anda dapat membuat Kartu Kendali untuk setiap unit
                    </p>
    ...
    <script>
        ...
        function convertSvgDataUriToPng(svgDataUri, fileName) {
            ...
            img.onerror = () => alert('Gagal memproses QR code.');
            img.src = svgDataUri;
        }
