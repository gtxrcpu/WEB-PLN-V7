<x-layouts.app :title="'Manage Floor Plans — Admin'">
  ...
  <div class="mb-4 sm:mb-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      ...
      <a href="{{ route('admin.floor-plans.create') }}"
        class="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl hover:from-blue-700 hover:to-indigo-700 transition-all shadow-lg hover:shadow-xl font-medium">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    @forelse($floorPlans as $floorPlan)
      <div class="bg-white rounded-xl shadow-lg ring-1 ring-slate-200 overflow-hidden hover:shadow-xl transition-all">
        ...
        <div class="relative h-48 bg-gray-100 overflow-hidden">
          <img src="{{ $floorPlan->image_url }}" alt="{{ $floorPlan->name }}" class="w-full h-full object-contain">

          {{-- Status Badge --}}
          <div class="absolute top-3 right-3">
            @if($floorPlan->is_active)
              <span
                class="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-semibold rounded-lg bg-green-100 text-green-700 ring-1 ring-green-200">
            ...
            @else
              <span
                class="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-semibold rounded-lg bg-gray-100 text-gray-700 ring-1 ring-gray-200">
                Inactive
              </span>
            @endif
          </div>
        ...
        <div class="p-4">
          ...
          <div class="flex items-center gap-2 mb-3">
            <span
              class="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold rounded-lg 
                {{ $floorPlan->unit?->code === 'INDUK' ? 'bg-gradient-to-r from-purple-100 to-violet-100 text-purple-700 ring-1 ring-purple-200' : 'bg-gradient-to-r from-emerald-100 to-teal-100 text-emerald-700 ring-1 ring-emerald-200' }}">
          ...
          <div class="flex items-center gap-2">
            ...
            <a href="{{ route('admin.floor-plans.edit', $floorPlan) }}"
              class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-all" title="Edit Floor Plan">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
              </svg>
            </a>
            <form action="{{ route('admin.floor-plans.destroy', $floorPlan) }}" method="POST"
              onsubmit="return confirm('⚠️ Yakin hapus floor plan {{ $floorPlan->name }}?\n\nDenah akan dihapus permanen!')">
              @csrf
              @method('DELETE')
