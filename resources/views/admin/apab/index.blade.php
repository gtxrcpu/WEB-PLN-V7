<x-layouts.app :title="'Admin - Manage APAB'">
    ...
    <div class="space-y-6">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="flex items-center gap-3">
          <a href="{{ route('admin.apab.create') }}"
      ...
      {{-- Stats Cards --}}
      @php
        $totalApab = $apabs->count();
        $statusBaik = $apabs->where('status', 'baik')->count();
        $statusPerbaikan = $apabs->where('status', 'perbaikan')->count();
        $statusRusak = $apabs->where('status', 'rusak')->count();
      @endphp
    ...
    @if($apabs->count())
      <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        @foreach($apabs as $apab)
          ...
          <div data-item data-serial="{{ $apab->serial_no }}" data-barcode="{{ $apab->barcode }}"
            ...
            <div class="p-6">
              ...
              <div class="mb-4">
                <span
                  class="inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1 text-xs font-semibold 
                          {{ $apab->unit?->code === 'INDUK' ? 'bg-purple-100 text-purple-700 ring-1 ring-purple-200' : 'bg-blue-100 text-blue-700 ring-1 ring-blue-200' }}">
                  <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd"
                      d="M4 4a2 2 0 012-2h8a2 2 0 012 2v12a1 1 0 110 2h-3a1 1 0 01-1-1v-2a1 1 0 00-1-1H9a1 1 0 00-1 1v2a1 1 0 01-1 1H4a1 1 0 110-2V4zm3 1h2v2H7V5zm2 4H7v2h2V9zm2-4h2v2h-2V5zm2 4h-2v2h2V9z"
                      clip-rule="evenodd" />
                  </svg>
                  {{ $apab->unit?->name ?? 'Induk' }}
                </span>
              </div>
              ...
              <div class="relative mb-5">
                ...
                <div
                  ...
                  <div class="mb-3">
                    <img src="{{ $apab->qr_url }}" alt="QR Apab {{ $kodePendek }}"
              ...
              <div class="flex flex-col gap-2">
                ...
                <a href="{{ route('apab.riwayat', $apab) }}"
                  ...
                  @php
                    $jumlahKartu = $apab->kartuApabs()->count();
                  @endphp
                  @if($jumlahKartu > 0)
                ...
                <a href="{{ route('admin.apab.edit', $apab) }}"
                ...
                <form method="POST" action="{{ route('admin.apab.destroy', $apab) }}"
                  onsubmit="return confirm('Yakin ingin menghapus Apab {{ $apab->serial_no }}? Data yang terhapus tidak dapat dikembalikan!');"
                  class="w-full">
                  @csrf
                  @method('DELETE')
                  <button type="submit"
      ...
      <div
        ...
        <div class="relative">
          ...
          <p class="text-slate-600 text-sm mb-6">
            Mulai kelola Apab dengan menambahkan unit pertama
          </p>
