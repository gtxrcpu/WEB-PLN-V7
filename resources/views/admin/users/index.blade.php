<x-layouts.app :title="'Manage Users — Admin'">
  ...
  <div class="mb-4 sm:mb-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      ...
      <a href="{{ route('admin.users.create') }}" class="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-xl hover:from-purple-700 hover:to-indigo-700 transition-all shadow-lg hover:shadow-xl font-medium">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <table class="w-full">
        ...
        <tbody class="divide-y divide-slate-200">
          @forelse($users as $user)
            <tr class="hover:bg-gradient-to-r hover:from-purple-50/50 hover:to-indigo-50/50 transition-all border-b border-slate-100 last:border-0">
              <td class="px-4 sm:px-6 py-4">
                <div class="flex items-center gap-3">
                  <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-purple-500 to-indigo-500 flex items-center justify-center text-white font-bold text-lg shadow-lg">
              ...
              <td class="px-4 sm:px-6 py-4 text-right">
                <div class="flex items-center justify-end gap-2">
                  ...
                  @if($user->id !== auth()->id())
                    <form action="{{ route('admin.users.destroy', $user) }}" method="POST" onsubmit="return confirm('⚠️ Yakin hapus user {{ $user->name }}?\n\nData user akan dihapus permanen!')">
                      @csrf
                      @method('DELETE')
                      <button type="submit" 
