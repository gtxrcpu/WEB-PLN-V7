<x-layouts.app :title="'Tambah Fire Alarm — Admin'">
  <div class="max-w-4xl mx-auto px-4 py-6">
    
    {{-- Header Card --}}
    <div class="relative overflow-hidden rounded-3xl bg-gradient-to-r from-red-600 to-orange-600 p-8 mb-6 shadow-2xl">
      ...
      <div class="relative flex items-center justify-between">
        <div class="flex items-center gap-4">
          ...
          <div>
            <h1 class="text-3xl font-bold text-white">Tambah Fire Alarm Baru</h1>
            <p class="text-white/80 text-sm mt-1">Input data Fire Alarm baru. Serial akan otomatis dibuat.</p>
          </div>
        </div>
    ...
    <div class="bg-white rounded-3xl shadow-xl border-2 border-slate-100 overflow-hidden">
      ...
      <div class="bg-gradient-to-r from-slate-50 to-slate-100 px-8 py-6 border-b-2 border-slate-100">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="text-xl font-bold text-slate-900">Informasi Fire Alarm</h2>
            <p class="text-sm text-slate-600">Lengkapi form di bawah untuk menambahkan Fire Alarm</p>
          </div>
        </div>
      </div>

      <form action="{{ route('admin.fire-alarm.store') }}" method="POST" class="p-8 space-y-6">
        @csrf
        ...
        <div class="space-y-2">
          ...
          <p class="flex items-center gap-1.5 text-xs text-slate-500">
            ...
            Serial otomatis: FA.xxx (increment). QR Code akan mengikuti serial ini.
          </p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="space-y-2">
            ...
            <input type="text" 
                   ...
                   class="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all @error('location_code') border-red-500 @enderror">
            @error('location_code')
          ...
          <div class="space-y-2">
            ...
            <input type="text" 
                   ...
                   class="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all @error('type') border-red-500 @enderror">
            @error('type')
              <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
            @enderror
          </div>
        ...
        <div class="space-y-2">
          ...
          <textarea name="notes" 
                    ...
                    class="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all resize-none">{{ old('notes') }}</textarea>
        </div>

        {{-- Action Buttons --}}
        <div class="flex items-center gap-3 pt-6 border-t-2 border-slate-100">
          <button type="submit" 
                  class="flex-1 inline-flex items-center justify-center gap-2 px-6 py-4 rounded-xl bg-gradient-to-r from-red-600 to-orange-600 text-white text-sm font-bold hover:from-red-700 hover:to-orange-700 shadow-lg shadow-red-500/30 hover:shadow-xl hover:shadow-red-500/40 transition-all duration-300">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
            </svg>
            <span>Simpan Fire Alarm</span>
          </button>
          <a href="{{ route('admin.fire-alarm.index') }}"
