<x-layouts.app :title="'Tambah APAT — Admin'">
    ...
    <div class="relative overflow-hidden rounded-3xl bg-gradient-to-r from-blue-600 to-cyan-600 p-8 mb-6 shadow-2xl">
      ...
      <div class="relative flex items-center justify-between">
        <div class="flex items-center gap-4">
          ...
          <div>
            <h1 class="text-3xl font-bold text-white">Tambah APAT Baru</h1>
            <p class="text-white/80 text-sm mt-1">Input data APAT baru. Serial akan otomatis dibuat.</p>
          </div>
        </div>
    ...
    <div class="bg-white rounded-3xl shadow-xl border-2 border-slate-100 overflow-hidden">
      ...
      <div class="bg-gradient-to-r from-slate-50 to-slate-100 px-8 py-6 border-b-2 border-slate-100">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="text-xl font-bold text-slate-900">Informasi APAT</h2>
            <p class="text-sm text-slate-600">Lengkapi form di bawah untuk menambahkan APAT</p>
          </div>
        </div>
      </div>

      <form action="{{ route('admin.apat.store') }}" method="POST" class="p-8 space-y-6">
        @csrf
        ...
        <div class="space-y-2">
          ...
          <p class="flex items-center gap-1.5 text-xs text-slate-500">
            ...
            Serial otomatis: A2.xxx (increment). QR Code akan mengikuti serial ini.
          </p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="space-y-2">
            ...
            <input type="text" 
                   ...
                   required
                   placeholder="BDG"
                   class="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition-all @error('lokasi') border-red-500 @enderror">
            @error('lokasi')
          ...
          <div class="space-y-2">
            ...
            <input type="text" 
                   ...
                   class="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition-all @error('jenis') border-red-500 @enderror">
            @error('jenis')
              <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
            @enderror
          </div>
        ...
        <div class="space-y-2">
          ...
          <input type="text" 
                 ...
                 class="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition-all @error('kapasitas') border-red-500 @enderror">
          @error('kapasitas')
            <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
          @enderror
        </div>
