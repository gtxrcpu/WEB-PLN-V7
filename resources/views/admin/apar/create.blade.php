    ...
    <div class="relative overflow-hidden rounded-3xl bg-gradient-to-r from-blue-600 to-cyan-600 p-8 mb-6 shadow-2xl">
      ...
      <div class="relative flex items-center justify-between">
        <div class="flex items-center gap-4">
          ...
          <div>
            <h1 class="text-3xl font-bold text-white">Tambah APAR Baru</h1>
            <p class="text-white/80 text-sm mt-1">Input data APAR baru. Serial akan otomatis dibuat.</p>
          </div>
        </div>
    ...
    <div class="bg-white rounded-3xl shadow-xl border-2 border-slate-100 overflow-hidden">
      ...
      <div class="bg-gradient-to-r from-slate-50 to-slate-100 px-8 py-6 border-b-2 border-slate-100">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="text-xl font-bold text-slate-900">Informasi APAR</h2>
            <p class="text-sm text-slate-600">Lengkapi form di bawah untuk menambahkan APAR</p>
          </div>
        </div>
      </div>

      <form action="{{ route('admin.apar.store') }}" method="POST" class="p-8 space-y-6">
        @csrf
        ...
        <div class="space-y-2">
          ...
          <div class="relative">
            <input type="text" 
                   value="{{ $nextSerial }}" 
                   readonly
                   class="w-full px-4 py-3 bg-gradient-to-br from-slate-50 to-slate-100 border-2 border-slate-200 rounded-xl text-slate-900 font-semibold text-lg">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="space-y-2">
            ...
            <input type="text" 
                   ...
                   required
                   placeholder="BDG"
                   class="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all @error('location_code') border-red-500 @enderror">
            @error('location_code')
          ...
          <div class="space-y-2">
            ...
            <input type="text" 
                   ...
                   required
                   placeholder="UUV"
                   class="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all @error('type') border-red-500 @enderror">
            @error('type')
              <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
            @enderror
          </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="space-y-2">
            ...
            <input type="text" 
                   ...
                   required
                   placeholder="5 Liter"
                   class="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all @error('capacity') border-red-500 @enderror">
            @error('capacity')
              <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
            @enderror
          </div>
        ...
        <div class="space-y-3">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <label class="relative cursor-pointer group">
              <input type="radio" name="status" value="isi ulang" {{ old('status') === 'isi ulang' ? 'checked' : '' }} class="peer sr-only">
              <div class="p-6 rounded-2xl border-2 border-slate-200 bg-white peer-checked:border-amber-500 peer-checked:bg-amber-50 hover:border-amber-300 transition-all duration-200">
                <div class="flex flex-col items-center gap-3">
                  <div class="w-12 h-12 rounded-full bg-amber-100 peer-checked:bg-amber-500 flex items-center justify-center transition-colors">
                    <svg class="w-6 h-6 text-amber-600 peer-checked:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                    </svg>
                  </div>
                  <span class="text-sm font-semibold text-slate-700 peer-checked:text-amber-700">ISI ULANG</span>
                </div>
              </div>
            </label>
        ...
        <div class="space-y-2">
          ...
          <textarea name="notes" 
                    ...
                    class="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all resize-none">{{ old('notes') }}</textarea>
        </div>
