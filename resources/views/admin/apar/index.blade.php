<x-layouts.app :title="'Admin - Manage APAR'">
    ...
    <div class="space-y-6">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="flex items-center gap-3">
          <a href="{{ route('admin.apar.create') }}"
          ...
          <div data-item data-serial="{{ $apar->serial_no }}" data-barcode="{{ $apar->barcode }}"
            ...
            <div class="p-6">
              ...
              <div class="mb-4">
                <span
                  class="inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1 text-xs font-semibold 
                      {{ $apar->unit?->code === 'INDUK' ? 'bg-purple-100 text-purple-700 ring-1 ring-purple-200' : 'bg-blue-100 text-blue-700 ring-1 ring-blue-200' }}">
                  <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd"
                      d="M4 4a2 2 0 012-2h8a2 2 0 012 2v12a1 1 0 110 2h-3a1 1 0 01-1-1v-2a1 1 0 00-1-1H9a1 1 0 00-1 1v2a1 1 0 01-1 1H4a1 1 0 110-2V4zm3 1h2v2H7V5zm2 4H7v2h2V9zm2-4h2v2h-2V5zm2 4h-2v2h2V9z"
                      clip-rule="evenodd" />
                  </svg>
                  {{ $apar->unit?->name ?? 'Induk' }}
                </span>
              </div>
              ...
              <div class="flex flex-col gap-2">
                ...
                <a href="{{ route('admin.apar.show', $apar) }}"
                  ...
                  @endphp
                  @if($jumlahKartu > 0)
                    <span class="ml-1 px-2 py-0.5 bg-blue-200 text-blue-800 rounded-full text-xs font-bold">
                      {{ $jumlahKartu }}
                    </span>
                  @endif
                </a>

                <a href="{{ route('admin.apar.edit', $apar) }}"
                ...
                <form method="POST" action="{{ route('admin.apar.destroy', $apar) }}"
                  onsubmit="return confirm('Yakin ingin menghapus APAR {{ $apar->serial_no }}? Data yang terhapus tidak dapat dikembalikan!');"
                  class="w-full">
                  @csrf
                  @method('DELETE')
                  <button type="submit"
      ...
      <div
        ...
        <div class="relative">
          ...
          <p class="text-slate-600 text-sm mb-6">
            Mulai kelola APAR dengan menambahkan unit pertama
          </p>
